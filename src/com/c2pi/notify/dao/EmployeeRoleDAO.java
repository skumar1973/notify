...
public class EmployeeRoleDAO {
	...
	public String addEmployeeRole(EmployeeRole empRole) throws SQLException,
		...
		conn = con.getConn();
		if (empRole.getId() == 0) {
			query = "Insert Into `c2pidb`.`emp_roles` (`emp_id`,`role_id`,`created_by`, " +
					"`created_dt`, `updated_by`, `updated_dt`) values(?,?,?,?,?,?)";
		} else {
			query = "Update emp_roles set emp_id=?,role_id=?,created_by=?, created_dt=?, " +
		...
		return (queryResult + "- Employee role added /updated.");
	}

	public EmployeeRole getEmployeeById(int empID) throws SQLException,
			ClassNotFoundException, IOException {
		empRole = new EmployeeRole();
		query = "SELECT id, emp_id, role_id, created_dt, created_by, updated_dt, updated_by FROM c2pidb.emp_roles where id=?";
		con = new DBConn();
		conn = con.getConn();
		pstmt = conn.prepareStatement(query);
	...
	public String deleteEmpRole(int emproleID)
			throws SQLException, ClassNotFoundException, IOException {
		
		query = "DELETE from `c2pidb`.`emp_roles` where id=?";
		con = new DBConn();
		conn = con.getConn();

		pstmt = conn.prepareStatement(query);
		pstmt.setInt(1, emproleID);
		System.out.println("pstmt" + pstmt);
